...
function updateProgress() {
  ...
  const oldScore = appState.lucidityScore || 0;
  ...
  if (progressPercent > oldScore) {
    animateScoreIncrease(oldScore, progressPercent);
  }
...
function triggerCelebration() {
  const colors = ['#E85B8A', '#7B4BA8', '#2D9CFF', '#FFB84D', '#52C77A'];
...
function createParticle(color) {
  ...
  particle.animate([
    ...
    { transform: `translateY(${targetY}px) rotate(${rotation}deg)`, opacity: 0 }
  ], {
    duration: duration,
